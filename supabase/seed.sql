...
BEGIN
    ...
    INSERT INTO public.categories (id, name, description, user_id) VALUES
        (gen_random_uuid(), 'Electronics', 'Electronic devices and gadgets', test_user_id),
        ...
        (gen_random_uuid(), 'Sports', 'Sports equipment and accessories', test_user_id)
    ...
    SELECT 
        ...
        test_user_id
    FROM (VALUES
        ('Laptop', 'High-performance laptop computer', 999.99),
        ...
        ('Running Shoes', 'Comfortable running shoes', 79.99)
    ...
    CROSS JOIN LATERAL (
        SELECT * FROM (VALUES
            (1, (SELECT price FROM public.products WHERE id = p.id LIMIT 1)),
            (2, (SELECT price FROM public.products WHERE id = p.id LIMIT 1))
        ) AS q(quantity, price)
        LIMIT 1
    ) AS oi
